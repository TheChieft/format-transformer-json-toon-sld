  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 md:p-8 border border-gray-200 dark:border-gray-700 transition-colors duration-300 shadow-lg">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-gray-300 dark:border-gray-600">
              ...
              <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">Structure</th>
              <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">Token Efficiency</th>
              <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">Readability</th>
              <th className="text-left py-3 px-4 font-semibold text-gray-700 dark:text-gray-300">Best For</th>
            </tr>
          </thead>
          <tbody>
            {formats.map((format) => (
              <tr key={format.name} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <td className="py-4 px-4">
                  <div className="flex items-center space-x-2">
                    <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${format.color}`}></div>
                    <span className="font-semibold text-gray-900 dark:text-gray-100">{format.name}</span>
                  </div>
                </td>
                ...
                <td className="py-4 px-4 text-gray-600 dark:text-gray-400">{format.tokenEfficiency}</td>
                <td className="py-4 px-4 text-gray-600 dark:text-gray-400">{format.readability}</td>
                <td className="py-4 px-4 text-gray-600 dark:text-gray-400 text-sm">{format.bestFor}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="lg:hidden space-y-4">
        {formats.map((format) => (
          <div key={format.name} className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${format.color}`}></div>
                <h3 className="font-bold text-lg text-gray-900 dark:text-gray-100">{format.name}</h3>
              </div>
      ...
      <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">💡 Choosing the Right Format</h3>
        <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
          <li>• <strong>LLM Prompts</strong>: Use TOON for 40% token savings</li>
          <li>• <strong>APIs & Web</strong>: Use JSON for universal compatibility</li>
          <li>• <strong>Flat Tables</strong>: Use CSV for maximum simplicity</li>
          <li>• <strong>Configuration</strong>: Use YAML for human editability</li>
          <li>• <strong>Unix Pipelines</strong>: Use TSV for command-line tools</li>
        </ul>
      </div>
    </div>
  );
